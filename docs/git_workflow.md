...
**Note:** If you want to contribute improvements to the course materials themselves (fix typos, improve examples, etc.), you should **fork** the repository instead of using it as a template. Forks maintain a connection to the original repository and allow you to submit pull requests with your improvements!
...
Creating a pull request is a great way to:
- Trigger automated CI checks (formatting, tests, warnings)
- Practice professional code review workflow
- Get feedback from instructors or peers

**To create a PR to your own main branch:**

1. Go to your repository on GitHub: `https://github.com/YOUR_USERNAME/cpp-quest`
2. You'll see a banner: "session-01 had recent pushes" with **"Compare & pull request"** button
3. Click it
4. **Important:** Make sure the base repository is set to **your own repository** (not the template):
   - Base repository: `YOUR_USERNAME/cpp-quest`
   - Base branch: `main`
   - Compare branch: `session-01`
...
This triggers GitHub Actions to run automated checks (if configured). You can review your own PR and merge it when ready!

**Alternative:** You can also trigger CI checks by simply pushing to any branch. PRs are just one convenient way to review and validate your work.

### Step 6: Merge Your Work

When you're satisfied with your implementation:

**If you created a PR:**
1. Review the CI checks (if configured)
2. Click "Merge pull request" on GitHub
3. Delete the branch (optional)

**If you worked directly:**
```bash
# Go back to main
git checkout main
...
# Create branch for next session
git checkout -b session-02
```
